...
@Component({
      ...
      <div class="modal" (click)="$event.stopPropagation()">
        ...
        <form [formGroup]="form" (ngSubmit)="guardar()">
          <div class="fgrid">
            ...
            @if (!editando) {
              <div class="f" [class.has-error]="fi('password')">
  ...
  styles: [\`
    ...
    .act-btn.edit:hover{border-color:#4f8ef7}
    .act-btn.key:hover{border-color:#f7c948}
...
export class UsuariosComponent implements OnInit {
  ...
  cambiarPass(): void {
    ...
    this.http.put(environment.apiUrl + '/usuarios/' + this.showPass.id + '/password', this.passForm.value).subscribe({
      ...
      error: (e) => { this.savingPass = false; this.passErr = e.error?.error || 'Error'; this.cdr.detectChanges(); }
    });
  }
}`;
...
// ─── 2. CAMBIAR CONTRASEÑA PROPIA (en el layout/perfil) ──────────────
// Agrega un botón en el layout para cambiar la contraseña propia
let layout = fs.readFileSync('src/app/layout/layout.component.ts', 'utf8');

// Verificar si ya tiene cambiarPass
if (!layout.includes('cambiarPass')) {
  // Agregar modal de cambio de contraseña al layout
  layout = layout.replace(
    `<button class="logout-btn" (click)="logout()">Salir</button>`,
    `<button class="logout-btn" (click)="showPass=true;cdr.detectChanges()">🔑 Contraseña</button>
      <button class="logout-btn" (click)="logout()">Salir</button>`
  );

  // Agregar el modal antes del cierre del template
  layout = layout.replace(
    `\`,\n  styles:`,
    `
  @if (showPass) {
    <div class="overlay" (click)="showPass=false;cdr.detectChanges()">
      <div class="modal" (click)="$event.stopPropagation()">
        <div class="modal-head"><h2>Cambiar mi contraseña</h2><button class="close-btn" (click)="showPass=false;cdr.detectChanges()">✕</button></div>
        <div class="fgrid">
          <div class="f full"><label>Contraseña actual</label><input type="password" [(ngModel)]="passActual" placeholder="Contraseña actual"></div>
          <div class="f full"><label>Nueva contraseña</label><input type="password" [(ngModel)]="passNueva" placeholder="Mínimo 6 caracteres"></div>
        </div>
        @if (passErr) { <div class="form-alert">❌ {{ passErr }}</div> }
        @if (passOk) { <div class="form-ok">✅ Contraseña actualizada</div> }
        <div class="mfooter">
          <button class="btn-cancel" (click)="showPass=false;cdr.detectChanges()">Cancelar</button>
          <button class="btn-save" (click)="cambiarPass()" [disabled]="savingPass">{{ savingPass ? 'Guardando...' : 'Actualizar' }}</button>
        </div>
      </div>
    </div>
  }
  ...
  // Agregar imports y variables al componente
  layout = layout.replace(
    `import { Component, inject } from '@angular/core';`,
    `import { Component, inject, ChangeDetectorRef } from '@angular/core';\nimport { FormsModule } from '@angular/forms';\nimport { HttpClient } from '@angular/common/http';\nimport { environment } from '../../environments/environment';`
  );

  // Agregar FormsModule a imports del componente
  layout = layout.replace(
    /imports:\s*\[([^\]]+)\]/,
    (match: string, inner: string) => {
      if (inner.includes('FormsModule')) return match;
      return match.replace(inner, inner.trimEnd() + ', FormsModule');
    }
  );

  // Agregar variables y método al componente
  layout = layout.replace(
  ...
  layout = layout.replace(
    `logout(): void {`,
    `cambiarPass(): void {
    if (!this.passNueva || this.passNueva.length < 6) { this.passErr = 'La nueva contraseña debe tener al menos 6 caracteres'; return; }
    this.savingPass = true; this.passErr = '';
    const token = localStorage.getItem('cobros_token');
    const payload = token ? JSON.parse(atob(token.split('.')[1])) : null;
    if (!payload) return;
    this.http.put(environment.apiUrl + '/usuarios/' + payload.id + '/password', { password_actual: this.passActual, password_nuevo: this.passNueva }).subscribe({
      next: () => { this.savingPass = false; this.passOk = true; this.passActual = ''; this.passNueva = ''; this.cdr.detectChanges(); setTimeout(() => { this.showPass = false; this.passOk = false; this.cdr.detectChanges(); }, 2000); },
      error: (e: any) => { this.savingPass = false; this.passErr = e.error?.error || 'Error al cambiar contraseña'; this.cdr.detectChanges(); }
    });
  }
  ...
  // Agregar estilos del modal al layout
  layout = layout.replace(
    `.logout-btn{`,
    `.overlay{position:fixed;inset:0;background:rgba(0,0,0,.65);display:flex;align-items:center;justify-content:center;z-index:200;backdrop-filter:blur(2px)}
    .modal{background:#1e2230;border:1px solid #2e3450;border-radius:16px;padding:24px;width:100%;max-width:440px}
    .modal-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:18px}
    .modal-head h2{font-size:.92rem;font-weight:700}
    .close-btn{background:none;border:none;color:#7a839e;font-size:1.1rem;cursor:pointer}
    .fgrid{display:grid;gap:12px}
    .f{display:flex;flex-direction:column;gap:5px}.f.full{grid-column:1/-1}
    label{font-size:.65rem;color:#7a839e;text-transform:uppercase;letter-spacing:.4px}
    input{background:#0d0f14;border:1px solid #2e3450;border-radius:8px;color:#e8ecf4;padding:8px 11px;font-size:.82rem;outline:none;font-family:inherit;width:100%;box-sizing:border-box}
    input:focus{border-color:#4f8ef7}
    .form-alert{background:rgba(247,95,95,.1);border:1px solid rgba(247,95,95,.3);border-radius:8px;padding:9px 12px;font-size:.75rem;color:#f75f5f;margin-top:10px}
    .form-ok{background:rgba(34,211,160,.1);border:1px solid rgba(34,211,160,.3);border-radius:8px;padding:9px 12px;font-size:.75rem;color:#22d3a0;margin-top:10px}
    .mfooter{display:flex;justify-content:flex-end;gap:10px;margin-top:18px}
    .btn-cancel{background:none;border:1px solid #2e3450;color:#7a839e;border-radius:10px;padding:8px 16px;font-size:.8rem;cursor:pointer;font-family:inherit}
    .btn-save{background:linear-gradient(135deg,#4f8ef7,#7c5cfc);color:#fff;border:none;border-radius:10px;padding:8px 18px;font-size:.8rem;font-weight:600;cursor:pointer;font-family:inherit}
    .btn-save:disabled{opacity:.6;cursor:not-allowed}
    .logout-btn{`
  );

  fs.writeFileSync('src/app/layout/layout.component.ts', layout, 'utf8');
  console.log('Layout: cambiar contraseña OK');
} else {
  console.log('Layout: cambiar contraseña ya existe');
}

// ─── 3. AGREGAR RUTA /usuarios ──────────────────────────────────────
...
// ─── 4. AGREGAR ENLACE EN LAYOUT ──────────────────────────────────────
if (!layout.includes("'/usuarios'") && !layout.includes('"/usuarios"')) {
  let l2 = fs.readFileSync('src/app/layout/layout.component.ts', 'utf8');
  l2 = l2.replace(
    `{ label: 'Pagos',`,
    `{ label: 'Usuarios', path: '/usuarios' },
      { label: 'Pagos',`
  );
  fs.writeFileSync('src/app/layout/layout.component.ts', l2, 'utf8');
  console.log('Layout: enlace /usuarios OK');
}

console.log('\n✅ Frontend usuarios + cambiar contraseña listos');
console.log('Ahora crea la carpeta: src/app/pages/usuarios/');
